// TODO: migrate to opentelemetry-rust-contrib package when released
// Reason = Using the code directly because the package is not released, and cargo git import not possible because of conflicting packages
// Copy of `<https://github.com/open-telemetry/opentelemetry-rust-contrib/blob/main/opentelemetry-stackdriver/src/google_trace_context_propagator.rs>`
...
impl TextMapPropagator for GoogleTraceContextPropagator {
    ...
    fn extract_with_context(&self, cx: &Context, extractor: &dyn Extractor) -> Context {
        self.extract_span_context(extractor)
            .map_or_else(|()| cx.clone(), |sc| cx.with_remote_span_context(sc))
    }
